...
bool rrtMultConnectPlanner::plan()
{
    initialize();

    // First get simple paths
    getSimplePath();
    seperateSimplePaths();
    addToTree();
...
void rrtMultConnectPlanner::getSimplePath()
{
...
void rrtMultConnectPlanner::seperateSimplePaths()
{
    this->groupCount = 0;
...
void rrtMultConnectPlanner::seperatePath(vector<node> &path)
{
    vector<node> group;
...
void rrtMultConnectPlanner::addToTree()
{
...
double rrtMultConnectPlanner::getDistStartToEnd()
{
    double distance;
    for(size_t i=0;i<JOINTNUM;++i){
